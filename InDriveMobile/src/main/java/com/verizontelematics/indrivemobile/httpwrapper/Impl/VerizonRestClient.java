...
public class VerizonRestClient implements RestClient {
    ...
    private static final String TAG = VerizonRestClient.class.getCanonicalName();
    ...
    public void init(Context aContext, SSLSocketFactory sslSocketFactory) {
        Log.d(TAG, "init with sslSocketFactory ");
    ...
    public void init(Context context,org.apache.http.conn.ssl.SSLSocketFactory sslSocketFactory){
        HttpStack httpStack = createHttpStack(sslSocketFactory);
        requestQueue = Volley.newRequestQueue(context, httpStack);

        mainHandler = new Handler(Looper.getMainLooper());
//        Volley.newRequestQueue(context, new VerizonSSLHttpStack(context, requestType, userId, deviceName, registrationCode));
    }

    private HttpStack createHttpStack(SSLSocketFactory socketFactory) {
        if (socketFactory == null)
            return null;
        Log.d(TAG,"layer7 createHttpStack");
        VerizonHttpStack stack = new VerizonHttpStack(socketFactory);
        return stack;
    }

    private HttpStack createHttpStack(org.apache.http.conn.ssl.SSLSocketFactory socketFactory) {
        if (socketFactory == null)
            return null;
        Log.d(TAG,"layer7 createHttpStack");
        VerizonHttpsStack stack = new VerizonHttpsStack(socketFactory);
        return stack;
    }
    ...
    public static class CustomRequest extends StringRequest {
        ...
        public CustomRequest(int method, String url, Listener<String> listener,
            ...
            setRetryPolicy(new DefaultRetryPolicy(60000,0,0));
