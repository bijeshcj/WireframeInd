...
public class AppController {
    ...
    public void setGCMToken(String token){
        InDrivePreference.getInstance().setStringData(AppStateData.GCM_TOKEN,token);
    }

    public String getGCMToken(){
        return InDrivePreference.getInstance().getStringData(AppStateData.GCM_TOKEN,"");
    }
...
//        }
    ...
    public void setAccountStatus(String status){
        InDrivePreference.getInstance().setStringData(AppStateData.ACCOUNT_STATUS,status);
    }

    public String getAccountStatus(){
        return InDrivePreference.getInstance().getStringData(AppStateData.ACCOUNT_STATUS,"Active");
    }
...
//        ((AppStateData)mAppStateModel.getData()).set(AppStateData.VEHICLE_ID, vehicleId);
    ...
    public void setPassword(String password) {
        ...
        if (password.isEmpty())
            return;
        SecretKeySpec keySpec = new SecretKeySpec(FileUtils.readKeyContentFromAppInternal(IndriveApplication.getInstance()),"AES");
        password = CryptoWrapper.encrypt1(password, keySpec);

        ((AppStateData)mAppStateModel.getData()).set(AppStateData.PASSWORD, password);
        InDrivePreference.getInstance().setStringData(AppStateData.PASSWORD,password);
    }
    ...
    public String getPasword() {
        ...
        SecretKeySpec keySpec = new SecretKeySpec(FileUtils.readKeyContentFromAppInternal(IndriveApplication.getInstance()),"AES");
        if (obj != null) {
            return CryptoWrapper.decrypt1((String)obj,keySpec);
        }else{
            return CryptoWrapper.decrypt1(InDrivePreference.getInstance().getStringData(AppStateData.PASSWORD, ""),keySpec);
        }
    ...
    public boolean checkAlreadyRegistered(Context context,String userName){
        String fileName = userName+".p12";
        File file = new File(context.getFilesDir(), fileName);
        boolean retFlag = file.exists();
        Log.d(TAG,"checkAlreadyRegistered "+retFlag);
        return retFlag;
    }
    ...
    public boolean isAppBackground(){
        boolean retFlag = InDrivePreference.getInstance().getBooleanData(AppConstants.APP_BACKGROUND_STATE,false);
        return retFlag;
    }

    public void setAppBackground(boolean flag){
        InDrivePreference.getInstance().setBooleanData(AppConstants.APP_BACKGROUND_STATE,flag);
    }
