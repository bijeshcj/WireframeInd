...
import com.verizontelematics.indrivemobile.unistroke.util.Trigonometric;
...
//		foreach (Point p in points)
	...
	// assumes that each path has been re sampled to the same number of points at the same distance apart.
    ...
    public static Point getMostRemotePoint(Point aCenter, Vector<Point> aSamplesPoints){
        Point lDistantPoint = null;
        double max = 0;
        double distance = 0;
        for(Point lPoint:aSamplesPoints){
            distance = Distance(aCenter,lPoint);
            if(Math.abs(distance) > max){
                lDistantPoint = lPoint;
                max = Math.abs(distance);
            }
        }
        return lDistantPoint;
    }
