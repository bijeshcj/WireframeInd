...
public class DateDataFormat {
    ...
    @SuppressLint("SimpleDateFormat")
    private static final DateFormat mFormatter = new SimpleDateFormat("M/d/yy");
    ...
    public static String getMonth(long startDate,long endDate){
        SimpleDateFormat fmt = new SimpleDateFormat("MMMM yyyy");
        long[] weeksMillis = new long[7];
        String[] months = new String[7];
        for(int i=0;i<weeksMillis.length;i++){
            if(i == 0) {
                weeksMillis[i] = startDate;
                months[i] = fmt.format(weeksMillis[i]);
                continue;
            }
            if(i == weeksMillis.length - 1) {
                weeksMillis[i] = endDate;
                months[i] = fmt.format(weeksMillis[i]);
                break;
            }

            weeksMillis[i] = weeksMillis[i - 1] + 86400000;
            months[i] = fmt.format(weeksMillis[i]);
        ...
        if(months[0].equalsIgnoreCase(months[6]))
            return months[6];

        int count = 0;
        for(int i =0;i<months.length;i++){
            if(months[0].equalsIgnoreCase(months[i]))
                count++;
        }
