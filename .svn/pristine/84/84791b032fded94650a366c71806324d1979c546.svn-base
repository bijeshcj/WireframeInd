package com.verizontelematics.indrivemobile.connection.responselisteners;

import android.util.Log;

import com.android.volley.Response;
import com.verizontelematics.indrivemobile.connection.ConnectionRequest;
import com.verizontelematics.indrivemobile.connection.requests.RequestParams;
import com.verizontelematics.indrivemobile.connection.requests.RequestType;
import com.verizontelematics.indrivemobile.connection.responses.DevicePairResponse;


import org.json.JSONObject;

/**
 * Created by bijesh on 2/17/2015.
 */
public class DevicePairResponseListener implements Response.Listener<JSONObject> {
    private static final String TAG = DevicePairResponseListener.class.getCanonicalName();
    @Override
    public void onResponse(JSONObject jsonObject) {
       Log.d(TAG, "^^^^^^^^^^^^^^^^ onResponse received ");
        Log.d(TAG, "DevicePair response " + jsonObject.toString());
        Log.d(TAG,"DevicePair Response Data "+ DevicePairResponse.getInstance());
        new ConnectionRequest().requestSecuredConnection(new RequestParams(RequestType.GET_CERTIFICATE_ID));
    }
}
