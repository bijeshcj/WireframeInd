...
public class BarChart extends BarLineChartBase<BarData> {
    ...
    private static final String TAG = BarChart.class.getCanonicalName();
    ...
    private boolean[] disabledXValues = {false,false,false,false,false,false,false};
    private ArrayList<Boolean> mDisabledXValuesList = new ArrayList<Boolean>();
    ...
    private Paint mDisabledXValuesPaint;
    ...
    public BarChart(Context context, AttributeSet attrs) {
        super(context, attrs);
        mDisabledXValuesList = new ArrayList<Boolean>();
        for(int i=0;i<7;i++){
            mDisabledXValuesList.add(false);
        }
    }
...
//        disabled xValues paint
        mDisabledXValuesPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        mDisabledXValuesPaint.setStyle(Paint.Style.FILL);
        mDisabledXValuesPaint.setColor(Color.rgb(64,64,64));
        mDisabledXValuesPaint.setTextAlign(Paint.Align.CENTER);
        mDisabledXValuesPaint.setTextSize(Utils.convertDpToPixel(10f));

        disabledXValues = new boolean[]{false,false,false,false,false,false,false};
        // calculate3DColors();
    }
...
//                    mRenderPaint.setColor(dataSet.getColor(j)); bijesh
//                    mRenderPaint.setColor(getGradientPaint());
//                    Log.d(TAG,"$$$ rect "+mBarRect);
                    mDrawCanvas.drawRect(mBarRect, getGradientPaint(mBarRect));
    ...
    private Paint getGradientPaint(RectF rectF){
        LinearGradient gradient = new LinearGradient(rectF.left, rectF.right, rectF.bottom, rectF.top,0xFF5D5E60,   /* 0xFF000000*/
                0xFF959698/*0xFFFFFFFF */ , Shader.TileMode.CLAMP);/**/
        Paint p = new Paint();
        p.setDither(true);
...
//                Log.d(TAG,"$$$ xLabel in MPChart "+label);
...
//                Log.d(TAG,"$$$ disabledXValues array null "+(mDisabledXValuesList == null));
                boolean setDisableXValues = mDisabledXValuesList.get(i);
                if(setDisableXValues){
                    mDrawCanvas.drawText(label, position[0],
                            yPos,
                            mDisabledXValuesPaint);
                }else {
                    mDrawCanvas.drawText(label, position[0],
                            yPos,
                            mXLabelPaint);
    ...
    public void setDisabledDates(ArrayList<Boolean> flags){
        this.mDisabledXValuesList = flags;
    }
